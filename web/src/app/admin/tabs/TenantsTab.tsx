...
export function TenantsTab() {
  ...
  const availableModules = [
    { code: 'communiti', label: 'Communiti & Réseaux' },
    { code: 'woxxpay', label: 'WoxxPay (Paiements)' },
    { code: 'woxxship', label: 'WoxxShip (Transporteurs)' },
    { code: 'click_and_collect', label: 'Click & Collect' },
    { code: 'loyalty', label: 'Fidélité & Récompenses' },
    { code: 'advanced_analytics', label: 'Statistiques Avancées' },
  ];
                    ...
                    <div className="flex flex-wrap gap-1 max-w-xs">
                      {t.modules && t.modules.length > 0 ? (
                        t.modules.map((m) => (
                          <span
                            key={m}
                            className="px-2 py-0.5 rounded-md bg-slate-100 border border-slate-300 text-[10px] text-slate-800 font-mono font-bold"
                          >
                            {m}
                          </span>
                        ))
